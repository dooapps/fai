...
import 'transports/web_socket_graph_connector.dart';

import '../crdt/index.dart';
import '../types/fai_gun.dart';
import 'flutter_fai_link.dart';
import 'graph/fai_graph.dart';
import 'graph/fai_graph_utils.dart';
...
class FlutterFaiClient {
  GunGraph? graph;
  late FlutterGunOptions _opt;
  FlutterGunLink? linkClass;

  FlutterFaiClient({this.linkClass, FlutterGunOptions? flutterGunOptions}) {
    initializedClient(
  ...
  ///
  /// @param options
  FlutterFaiClient opt(FlutterGunOptions options) {
    _opt.merge(options);
